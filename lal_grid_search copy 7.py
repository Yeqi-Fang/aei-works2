...
N1s = [(N1, 8, 8) for N1 in Ns]
N2s = [(8, N2, 8) for N2 in Ns]
N3s = [(8, 8, N3) for N3 in Ns]
total_N = N1s + N2s + N3s

gamma1 = 8
gamma2 = 20

for (N1, N2, N3) in total_N:
    ...
    fig.tight_layout()
    fig.savefig(f"images/grid_num/mismatch_distribution_lal-{mf}-{mf1}-{mf2}-{N}-{depth}-{N1}-{N2}-{N3}.pdf")
    # plt.show()

    # rumtime
    # print("runtime: ", config.tau_total)
    # save mismatch to a csv 
    
    mismatch_df = pd.DataFrame(mismatches, columns=["mismatch"])
    mismatch_df.to_csv(os.path.join(outdir, f"data/mismatches_{N1}-{N2}-{N3}.csv"), index=False)
