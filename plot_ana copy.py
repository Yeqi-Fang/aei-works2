...
def read_data_from_file(path: str) -> np.ndarray:
    """return ndarray(N,4): freq, f1dot, f2dot, 2Fr"""
    rows = []
...
if __name__ == "__main__":
    DAT = ("LAL_example_data/LALSemiCoherentF0F1F2_analytical/"
           "dats/semicoh_results.dat")

    # plot_3D(DAT)
    data = read_data_from_file(DAT)
    # find the best 2Fr for a given (f, f1dot, f2dot)
    print(data[:, 3].min(), data[:, 3].max())
    interp, mins, span = build_interpolator(data, n_sample=200_000)
    
    twofr_perf = predict_2Fr(151.5, -1e-10, -1e-20, interp, mins, span)
    
    print(f"2Fr(151.5, -1e-10, -1e-20) = {twofr_perf:.6f}")
